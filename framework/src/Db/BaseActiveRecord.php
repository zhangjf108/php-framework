...
use Kerisy\Exception\UnknownMethodException;

/**
 * ActiveRecord is the base class for classes representing relational data in terms of objects.
 *
 * See [[\Kerisy\Db\ActiveRecord]] for a concrete implementation.
 *
...
abstract class BaseActiveRecord extends Model implements ActiveRecordInterface
{
    ...
    protected static function findByCondition($condition)
    {
        ...
        if (!ArrayUtil::isAssociative($condition)) {
     ...
     * @throws \Kerisy\Exception\InvalidParamException if relation name is wrong
    ...
    public function beforeSave($insert)
    {
        if ($insert) {
            if ($this->hasAttribute($this->createdAtAttribute)) {
                $this->setAttribute($this->createdAtAttribute, $this->timestamp ? time() : (new Carbon())->toDateTimeString());
            }

            if ($this->hasAttribute($this->updatedAtAttribute)) {
                $this->setAttribute($this->updatedAtAttribute, $this->timestamp ? time() : (new Carbon())->toDateTimeString());
            }
        } else {
            if ($this->hasAttribute($this->updatedAtAttribute)) {
                $this->setAttribute($this->updatedAtAttribute, $this->timestamp ? time() : (new Carbon())->toDateTimeString());
