...
use Kerisy;
use Kerisy\base\InvalidConfigException;
use Kerisy\helpers\FormatConverter;
...
class DateValidator extends Validator
{
     ...
     *
     * If this property is not set, the default value will be obtained from `Kerisy::$app->formatter->dateFormat`, see [[\Kerisy\i18n\Formatter::dateFormat]] for details.
     ...
     *
     * - if type is [[TYPE_DATE]], the default value will be taken from [[\Kerisy\i18n\Formatter::dateFormat]],
     * - if type is [[TYPE_DATETIME]], it will be taken from [[\Kerisy\i18n\Formatter::datetimeFormat]],
     * - and if type is [[TYPE_TIME]], it will be [[\Kerisy\i18n\Formatter::timeFormat]].
     *
     ...
     * This is only effective when the [PHP intl extension](http://php.net/manual/en/book.intl.php) is installed.
     * If not set, the locale of the [[\Kerisy\base\Application::formatter|formatter]] will be used.
     * See also [[\Kerisy\i18n\Formatter::locale]].
     */
    public $locale;
    /**
     ...
     * If this property is not set, [[\Kerisy\base\Application::timeZone]] will be used.
     */
    public $timeZone;
    /**
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Kerisy::t('Kerisy', 'The format of {attribute} is invalid.');
        }
        if ($this->format === null) {
            if ($this->type === self::TYPE_DATE) {
                $this->format = Kerisy::$app->formatter->dateFormat;
            } elseif ($this->type === self::TYPE_DATETIME) {
                $this->format = Kerisy::$app->formatter->datetimeFormat;
            } elseif ($this->type === self::TYPE_TIME) {
                $this->format = Kerisy::$app->formatter->timeFormat;
            } else {
                throw new InvalidConfigException('Unknown validation type set for DateValidator::$type: ' . $this->type);
            }
        }
        if ($this->locale === null) {
            $this->locale = Kerisy::$app->language;
        }
        if ($this->timeZone === null) {
            $this->timeZone = Kerisy::$app->timeZone;
        }
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = Kerisy::t('Kerisy', '{attribute} must be no less than {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = Kerisy::t('Kerisy', '{attribute} must be no greater than {max}.');
        }
    ...
    private function parseDateValueIntl($value, $format)
    {
        ...
        if ($parsedDate === false || $parsePos !== mb_strlen($value, Kerisy::$app ? Kerisy::$app->charset : 'UTF-8')) {
            return false;
        }
