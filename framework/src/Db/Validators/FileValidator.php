...
use Kerisy;
use Kerisy\helpers\FileHelper;
use Kerisy\helpers\Html;
use Kerisy\helpers\Json;
use Kerisy\web\JsExpression;
use Kerisy\web\UploadedFile;
...
class FileValidator extends Validator
{
     ...
     *   with [[\Kerisy\i18n\Formatter::asShortSize()|Formatter::asShortSize()]]
     */
    public $tooBig;
    /**
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Kerisy::t('Kerisy', 'File upload failed.');
        }
        if ($this->uploadRequired === null) {
            $this->uploadRequired = Kerisy::t('Kerisy', 'Please upload a file.');
        }
        if ($this->tooMany === null) {
            $this->tooMany = Kerisy::t('Kerisy', 'You can upload at most {limit, number} {limit, plural, one{file} other{files}}.');
        }
        if ($this->wrongExtension === null) {
            $this->wrongExtension = Kerisy::t('Kerisy', 'Only files with these extensions are allowed: {extensions}.');
        }
        if ($this->tooBig === null) {
            $this->tooBig = Kerisy::t('Kerisy', 'The file "{file}" is too big. Its size cannot exceed {formattedLimit}.');
        }
        if ($this->tooSmall === null) {
            $this->tooSmall = Kerisy::t('Kerisy', 'The file "{file}" is too small. Its size cannot be smaller than {formattedLimit}.');
        }
        ...
        if ($this->wrongMimeType === null) {
            $this->wrongMimeType = Kerisy::t('Kerisy', 'Only files with these MIME types are allowed: {mimeTypes}.');
        }
    ...
    protected function validateValue($value)
    {
        ...
        switch ($value->error) {
                ...
                if ($this->maxSize !== null && $value->size > $this->getSizeLimit()) {
                    return [
                        ...
                        [
                            ...
                            'formattedLimit' => Kerisy::$app->formatter->asShortSize($this->getSizeLimit()),
                        ],
                    ];
                    ...
                    return [
                        ...
                        [
                            'file' => $value->name,
                            'limit' => $this->minSize,
                            'formattedLimit' => Kerisy::$app->formatter->asShortSize($this->minSize),
                        ],
                    ];
            ...
            case UPLOAD_ERR_PARTIAL:
                Kerisy::warning('File was only partially uploaded: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
                Kerisy::warning('Missing the temporary folder to store the uploaded file: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_CANT_WRITE:
                Kerisy::warning('Failed to write the uploaded file to disk: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_EXTENSION:
                Kerisy::warning('File upload was stopped by some PHP extension: ' . $value->name, __METHOD__);
                break;
            default:
                break;
        }
    ...
    public function getSizeLimit()
    {
        ...
        if ($postLimit > 0 && $postLimit < $limit) {
            Kerisy::warning('PHP.ini\'s \'post_max_size\' is less than \'upload_max_filesize\'.', __METHOD__);
            $limit = $postLimit;
        }
    ...
    public function clientValidateAttribute($model, $attribute, $view)
    {
        ...
        return 'Kerisy.validation.file(attribute, messages, ' . Json::encode($options) . ');';
    }
     ...
     * @throws \Kerisy\base\InvalidConfigException
     * @see mimeTypes
     * @since 2.0.8
     */
