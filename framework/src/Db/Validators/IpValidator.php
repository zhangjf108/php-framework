...
class IpValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Kerisy::t('Kerisy', '{attribute} must be a valid IP address.');
        }
        if ($this->ipv6NotAllowed === null) {
            $this->ipv6NotAllowed = Kerisy::t('Kerisy', '{attribute} must not be an IPv6 address.');
        }
        if ($this->ipv4NotAllowed === null) {
            $this->ipv4NotAllowed = Kerisy::t('Kerisy', '{attribute} must not be an IPv4 address.');
        }
        if ($this->wrongCidr === null) {
            $this->wrongCidr = Kerisy::t('Kerisy', '{attribute} contains wrong subnet mask.');
        }
        if ($this->noSubnet === null) {
            $this->noSubnet = Kerisy::t('Kerisy', '{attribute} must be an IP address with specified subnet.');
        }
        if ($this->hasSubnet === null) {
            $this->hasSubnet = Kerisy::t('Kerisy', '{attribute} must not be a subnet.');
        }
        if ($this->notInRange === null) {
            $this->notInRange = Kerisy::t('Kerisy', '{attribute} is not in the allowed range.');
        }
    }
    ...
    public function clientValidateAttribute($model, $attribute, $view)
    {
        ...
        return 'Kerisy.validation.ip(value, messages, ' . Json::htmlEncode($options) . ');';
    }
