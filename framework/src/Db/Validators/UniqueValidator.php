...
use Kerisy;
use Kerisy\base\Model;
use Kerisy\db\ActiveQuery;
use Kerisy\db\ActiveQueryInterface;
use Kerisy\db\ActiveRecord;
use Kerisy\db\ActiveRecordInterface;
use Kerisy\helpers\Inflector;
...
class UniqueValidator extends Validator
{
    ...
    public $targetClass;
    /**
     * @var string|array the name of the [[\Kerisy\db\ActiveRecord|ActiveRecord]] attribute that should be used to
     ...
     * This can be a string or an array representing the additional query condition (refer to [[\Kerisy\db\Query::where()]]
     * on the format of query condition), or an anonymous function with the signature `function ($query)`, where `$query`
     * is the [[\Kerisy\db\Query|Query]] object that you can modify in the function.
     */
    public $filter;
    /**
    ...
    public function init()
    {
        ...
        if (is_array($this->targetAttribute) && count($this->targetAttribute) > 1) {
            ...
            if ($this->comboNotUnique === null) {
                $this->message = Kerisy::t('Kerisy', 'The combination {values} of {attributes} has already been taken.');
            } else {
                $this->message = $this->comboNotUnique;
            }
        } else {
            $this->message = Kerisy::t('Kerisy', '{attribute} "{value}" has already been taken.');
        }
    }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        foreach ($rawConditions as $key => $value) {
            if (is_array($value)) {
                $this->addError($model, $attribute, Kerisy::t('Kerisy', '{attribute} is invalid.'));
                return;
            }
     ...
     * @param array $conditions conditions, compatible with [[\Kerisy\db\Query::where()|Query::where()]] key-value format.
     * @param Model $model the data model to be validated
     *
    ...
    private function modelExists($targetClass, $conditions, $model)
    {
        ...
        if (!$model instanceof ActiveRecordInterface || $model->getIsNewRecord() || $model->className() !== $targetClass::className()) {
            ...
            if ($query instanceof \Kerisy\db\ActiveQuery) {
     ...
     * [[\Kerisy\db\Query::where()|Query::where()]] key-value format.
     *
     * @param string|array $targetAttribute the name of the [[\Kerisy\db\ActiveRecord|ActiveRecord]] attribute that
     ...
     *
     * @return array conditions, compatible with [[\Kerisy\db\Query::where()|Query::where()]] key-value format.
     */
     ...
     * @param \Kerisy\Model\Model $model the data model.
