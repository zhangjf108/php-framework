...
namespace Kerisy\Db\oci;

use Kerisy\Exception\InvalidCallException;
use Kerisy\Exception\NotSupportedException;
use Kerisy\Db\CheckConstraint;
use Kerisy\Db\ColumnSchema;
...
class Schema extends \Kerisy\Db\Schema
{
     ...
     */
    public $exceptionMap = [
        'ORA-00001: unique constraint' => 'Kerisy\Db\IntegrityException',
    ];
...
SQL;
        ...
        $indexes = ArrayUtil::index($indexes, null, 'name');
        $result = [];
        foreach ($indexes as $name => $index) {
            $result[] = new IndexConstraint([
                ...
                'name' => $name,
                'columnNames' => ArrayUtil::getColumn($index, 'column_name'),
            ]);
        }
        return $result;
    }
...
SQL;
     ...
     * @internal param \Kerisy\Db\TableSchema $table->name the table schema
                ...
                switch ($type) {
                    case 'P':
                        $result['primaryKey'] = new Constraint([
                            'name' => $name,
                            'columnNames' => ArrayUtil::getColumn($constraint, 'column_name'),
                        ]);
                        break;
                    case 'R':
                        $result['foreignKeys'][] = new ForeignKeyConstraint([
                            ...
                            'foreignColumnNames' => ArrayUtil::getColumn($constraint, 'foreign_column_name'),
