...
class Command extends Component
{
     ...
     * @param string $type the column type. [[\Kerisy\Db\QueryBuilder::getColumnType()]] will be called
    ...
    /**
     * Logs the current database query
     * @param float $start the query start time.
     * @return array
     */
    private function logQuery(float $start)
    {
        $elapsed = round((microtime(true) - $start) * 1000, 3);

        if ($this->db->enableLogging) {
            $rawSql = $this->getRawSql();
            Kerisy::info($rawSql, ['elapsed' => $elapsed]);
        }

        return [$rawSql, $elapsed];
    }
    ...
    protected function queryInternal($method, $fetchMode = null)
    {
        ...
        if ($method !== '') {
            ...
            if (is_array($info)) {
                /* @var $cache \Kerisy\Cache\Simple\AbstractCache */
                $cache = $info[0];
