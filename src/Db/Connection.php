 ...
 *
 * ```php
 * $connection = new \Kerisy\Db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
class Connection extends Component
{
     ...
     * @see enableSchemaCache
     */
    public $schemaCache = 'simpleCache';
    /**
     ...
     * schema class name or configuration. Please refer to [[Kerisy::createObject()]] for
     * details on how to specify a configuration.
     *
     * This property is mainly used by [[getSchema()]] when fetching the database schema information.
     * You normally do not need to set this property unless you want to use your own
     * [[Schema]] class to support DBMS that is not supported by Kerisy.
     */
    public $schemaMap = [
        'pgsql' => 'Kerisy\Db\pgsql\Schema', // PostgreSQL
        'mysqli' => 'Kerisy\Db\mysql\Schema', // MySQL
        'mysql' => 'Kerisy\Db\mysql\Schema', // MySQL
        'sqlite' => 'Kerisy\Db\sqlite\Schema', // sqlite 3
        'sqlite2' => 'Kerisy\Db\sqlite\Schema', // sqlite 2
        'sqlsrv' => 'Kerisy\Db\mssql\Schema', // newer MSSQL driver on MS Windows hosts
        'oci' => 'Kerisy\Db\oci\Schema', // Oracle driver
        'mssql' => 'Kerisy\Db\mssql\Schema', // older MSSQL driver on MS Windows hosts
        'dblib' => 'Kerisy\Db\mssql\Schema', // dblib drivers on GNU/Linux (and maybe other OSes) hosts
        'cubrid' => 'Kerisy\Db\cubrid\Schema', // CUBRID
    ];
    /**
     * @var string Custom PDO wrapper class. If not set, it will use [[PDO]] or [[\Kerisy\Db\mssql\PDO]] when MSSQL is used.
     * @see pdo
     */
    public $pdoClass;
    /**
     ...
     * @since 2.0.7
     */
    public $commandClass = 'Kerisy\Db\Command';
    /**
     ...
     */
    public $serverStatusCache = 'simpleCache';
    /**
    ...
    public function getQueryCacheInfo($duration)
    {
        ...
        if ($duration === 0 || $duration > 0) {
            if (is_string($this->queryCache) && Kerisy::$app) {
                $cache = Kerisy::$app->get($this->queryCache, false);
            } else {
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql' || $driver === 'dblib') {
                    $pdoClass = 'Kerisy\Db\mssql\PDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'Kerisy\Db\mssql\SqlsrvPDO';
                }
            }
        }
     ...
     * from rollback will be caught and just logged with [[\Kerisy::error()]].
    ...
    private function rollbackTransactionOnLevel($transaction, $level)
    {
        if ($transaction->isActive && $transaction->level === $level) {
            try {
                ...
                Kerisy::error($e->getMessage(), [$e->getCode(), $e->getFile(), $e->getLine()]);
            }
        }
    }
    ...
    public function getSchema()
    {
        ...
        if (isset($this->schemaMap[$driver])) {
            ...
            return $this->_schema = Kerisy::make($config);
        }
     ...
     * @return Connection the opened DB connection, or `null` if no server is available
     * @throws InvalidConfigException if a configuration does not specify "dsn"
     * @throws ConnectionException if a configuration does not specify "dsn"
     * @since 2.0.11
     */
    protected function openFromPoolSequentially(array $pool, array $sharedConfig)
    {
        ...
        $cache = is_string($this->serverStatusCache) ? Kerisy::$app->get($this->serverStatusCache, false) : $this->serverStatusCache;
        ...
        foreach ($pool as $config) {
            ...
            try {
                ...
                throw new ConnectionException("Connection ({$config['dsn']}) failed: " . $e->getMessage());
            }
        }
