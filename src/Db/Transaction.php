...
class Transaction extends Component
{
    ...
    public function begin($isolationLevel = null)
    {
        ...
        if ($this->_level === 0) {
            ...
            Kerisy::trace('Begin transaction' . ($isolationLevel ? ' with isolation level ' . $isolationLevel : ''), __METHOD__);
        ...
        if ($schema->supportsSavepoint()) {
            Kerisy::trace('Set savepoint ' . $this->_level, __METHOD__);
            $schema->createSavepoint('LEVEL' . $this->_level);
        } else {
            Kerisy::info('Transaction not started: nested transaction not supported', __METHOD__);
        }
        $this->_level++;
    }
    ...
    public function commit()
    {
        ...
        $this->_level--;
        if ($this->_level === 0) {
            Kerisy::trace('Commit transaction', __METHOD__);
            $this->db->pdo->commit();
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Kerisy::trace('Release savepoint ' . $this->_level, __METHOD__);
            $schema->releaseSavepoint('LEVEL' . $this->_level);
        } else {
            Kerisy::info('Transaction not committed: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function rollBack()
    {
        ...
        $this->_level--;
        if ($this->_level === 0) {
            Kerisy::trace('Roll back transaction', __METHOD__);
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Kerisy::trace('Roll back to savepoint ' . $this->_level, __METHOD__);
            $schema->rollBackSavepoint('LEVEL' . $this->_level);
        } else {
            Kerisy::info('Transaction not rolled back: nested transaction not supported', __METHOD__);
    ...
    public function setIsolationLevel($level)
    {
        ...
        Kerisy::trace('Setting transaction isolation level to ' . $level, __METHOD__);
