...
namespace Kerisy\Db\cubrid;

use Kerisy\Db\Constraint;
use Kerisy\Db\ConstraintFinderTrait;
use Kerisy\Db\Expression;
use Kerisy\Db\ForeignKeyConstraint;
use Kerisy\Db\IndexConstraint;
use Kerisy\Db\TableSchema;
use Kerisy\Db\Transaction;
 ...
 * @since 2.0
 */
class Schema extends \Kerisy\Db\Schema
{
     ...
     */
    public $exceptionMap = [
        'Operation would have caused one or more unique constraint violations' => 'Kerisy\Db\IntegrityException',
    ];
    ...
    protected function loadTablePrimaryKey($tableName)
    {
        ...
        ArrayUtil::multisort($primaryKey, 'KEY_SEQ', SORT_ASC, SORT_NUMERIC);
        return new Constraint([
            'name' => $primaryKey[0]['KEY_NAME'],
            'columnNames' => ArrayUtil::getColumn($primaryKey, 'ATTR_NAME'),
        ]);
    }
    ...
    protected function loadTableForeignKeys($tableName)
    {
        ...
        $foreignKeys = ArrayUtil::index($foreignKeys, null, 'FK_NAME');
        ArrayUtil::multisort($foreignKeys, 'KEY_SEQ', SORT_ASC, SORT_NUMERIC);
        $result = [];
        foreach ($foreignKeys as $name => $foreignKey) {
            $result[] = new ForeignKeyConstraint([
                'name' => $name,
                'columnNames' => ArrayUtil::getColumn($foreignKey, 'FKCOLUMN_NAME'),
                'foreignTableName' => $foreignKey[0]['PKTABLE_NAME'],
                'foreignColumnNames' => ArrayUtil::getColumn($foreignKey, 'PKCOLUMN_NAME'),
     ...
     * @return \Kerisy\Db\ColumnSchema the column schema object
     */
    ...
    private function loadTableConstraints($tableName, $returnType)
    {
        ...
        $constraints = ArrayUtil::index($constraints, null, ['TYPE', 'NAME']);
        ArrayUtil::multisort($constraints, 'KEY_ORDER', SORT_ASC, SORT_NUMERIC);
        $result = [
            'indexes' => [],
            'uniques' => [],
        ];
        foreach ($constraints as $type => $names) {
            foreach ($names as $name => $constraint) {
                ...
                $result['indexes'][] = new IndexConstraint([
                    ...
                    'isUnique' => $isUnique,
                    'name' => $name,
                    'columnNames' => ArrayUtil::getColumn($constraint, 'ATTR_NAME'),
                ]);
                if ($isUnique) {
